...
CORS は、バックエンドが特定の Origin の JavaScript に対して
API レスポンスや Cookie を含む通信結果をブラウザ上で扱うことを許可するための仕組みであり、API 自体のセキュリティを強化するものではありません。
...
もしブラウザに Same-Origin Policy が存在しなければ、
不正なサイトの JavaScript が正規サイトに対して
認証 Cookie を含むリクエストを送信し、
本来は認証ユーザーしか見られない情報を読み取れてしまう。

これを防ぐため、ブラウザにはデフォルトで
Same-Origin Policy が実装されており、
CORS はその制約をサーバが明示的に緩和するための仕組みである。
